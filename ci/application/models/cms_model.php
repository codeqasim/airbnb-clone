...
class Cms_model extends My_Model
{
	...
	function booked_rental($prd_id='')
		...
		$this->db->select('pb.*,pa.post_code,pa.address,pa.apt,
								...
								u.firstname,u.image,
								rq.id as EnqId,rq.booking_status,rq.checkin,rq.checkout,rq.dateAdded,rq.numofdates,rq.user_id as GestId,py.id as PaymentId,py.status as PaymentStatus,rq.serviceFee,rq.totalAmt,rq.Bookingno');
			$this->db->from(PRODUCT_BOOKING.' as pb');
