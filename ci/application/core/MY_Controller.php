...
class MY_Controller extends CI_Controller {  
	...
	function __construct()
    {
		...
		if ($this->data['cmsList'] == ''){
			...
			$this->data['cmsList'] = $this->db->query('select * from '.CMS.' where `lang_code`="en"');
			}else{
			$this->data['cmsList'] = $this->db->query('select * from '.CMS.' where `lang_code`="'.$this->session->userdata('language_code').'"');
			}
		}
