...
$( "#expiredateContact1" ).datepicker( "option", "minDate", selectedDate );

$( "#expiredateContact1" ).val('');
...
beforeShowDay: function(date){
	...
	onClose: function( selectedDate ) {
        $( "#datefromContact1" ).datepicker( "option", "maxDate", selectedDate );
      }
...
function DateBetweenContactNew(){

     var date = $("#datefromContact1").datepicker('getDate');
    //var start = date.setDate(date.getDate() + 1),
	var start = date.setDate(date.getDate()),
        end = $("#expiredateContact1").datepicker("getDate"),
        currentDate = new Date(start),       
        between = [];
    while (currentDate < end) {
        between.push(new Date(currentDate));
    ...
    $.ajax({
        ...
        //alert(response);
            if(response=='Date Already Booked'){
				...
				if($("#datefromContact1").val()!='' && $("#expiredateContact1").val()!='') {
					...
					$('#bookingsubtot1').html(newTotl);

					$('#bookingdate1').html("Booking for "+days+" Night");

					if(days > 1)

					$('#bookingdate1').html("Booking for "+days+" Nights");
					...
					var st = $('#servicetax1').html();

					var taxtype =$('#taxtype1').html();
                ...
                <?php if($product->home_type !='') {?>
                  ...
                  <?php } if($product->accommodates !='') {?>
					...
					<?php 
							...
							<?php if(strtolower($listingResult) == 'beds'){?>
							<li>

							 <i class="topsp spr3"></i>

							 <span><?php echo stripslashes(ucfirst($FinalValues)); ?> <?php if($this->lang->line('beds') != '') { echo stripslashes($this->lang->line('beds')); } else echo "Beds";?></span>
...
<!--......... .......... .......... FOR EMPTY SPACE....................-->
			 ...
			 <div class="animation-container">
                ...
                <div class="top-title">
                    ...
                    <span class="title"><?php echo $this->session->userdata('currency_s').' '.stripslashes($product->price) * $this->session->userdata('currency_r'); ?></span> <label class="price-descri"><?php if($this->lang->line('per_night') != '') { echo stripslashes($this->lang->line('per_night')); } else echo "per night"; ?></label>
              ...
              <input type="text" placeholder="dd-mm-yyyy" autocomplete="off"   class="login-popup" id="datefromCheck" />

              <?php }else{ ?>

              <input type="text" placeholder="dd-mm-yyyy" autocomplete="off" name="datefrom" readonly="readonly" id="datefrom" class="checkin ui-datepicker-target" />
              ...
              <?php } ?>  
                    ...
                    <li>
                        ...
                        <label><?php if($this->lang->line('checkout') != '') { echo stripslashes($this->lang->line('check_out')); } else echo "check out"; ?></label>

						<?php if($loginCheck==''){?>

              <input type="text" placeholder="dd-mm-yyyy" autocomplete="off" class="login-popup" id="expiredateCheck"/>

              <?php }else{ ?>

                        <input type="text" placeholder="dd-mm-yyyy" name="expiredate" autocomplete="off" readonly="readonly" id="expiredate" onchange="return DateBetween();" class="checkout ui-datepicker-target">
                ...
                <div class="service-copmity" id="price_div" style="display:none">
					...
					echo  "Booking for"; ?> 1 <?php if($this->lang->line('night') != '') { echo stripslashes($this->lang->line('night')); } else echo "Night"; ?>
...
<label class="price"><?php echo $this->session->userdata('currency_s'); ?></label><label id="bookingsubtot">
					...
					<?php //echo $product->price;?></label></li>
                     ...
                     <li><span><?php if($this->lang->line('service_fee') != '') { echo stripslashes($this->lang->line('service_fee')); } else echo "Service Fee"; ?></span>
                    ...
                    <li ><span><?php if($this->lang->line('total') != '') { echo stripslashes($this->lang->line('total')); } else echo "Total"; ?></span><label class="price"> <?php echo $this->session->userdata('currency_s'); ?></label><label id="bookingtot" class="table-cell-price">0.00</label></li>

					

					 <li style="display:none;"><span><?php if($this->lang->line('total') != '') { echo stripslashes($this->lang->line('total')); } else echo "Total"; ?></span><label id="bookingtot" class="table-cell-price"></label></li>
			...
			<article class="descri-section new-line">
				...
				<?php
                   ...
                   $list_name = $listDetail->row()->list_name;
					...
					<li <?php if($mcount>4){ echo 'class="amore'.$list->id.'" style="display:none"'; }?>><?php if($details->image != '') { ?><img src="images/attribute/<?php echo $details->image;?>"<?php   } else {?> <img src="images/attribute/default1.png" <?php }  ?> style="height:25px;width:25px;float:left;"/><label <?php if(!in_array($key,$sub_list)){ echo 'style="text-decoration:line-through"';} ?>><?php echo $details->list_value;?></label></li>
...
<?php echo $total_review;?> <?php if($this->lang->line('review') != '') { echo stripslashes($this->lang->line('review')); } else echo "Review";?><span style="padding-right: 10px;"></span><span class="review_img"><span class="review_st" style="width:<?php echo $reviewTotal->row()->tot_tot * 20?>%"></span></span>
...
<div class="container" id="host">
                      ...
                      -->  <?php } ?>
                       ...
                       <!-- <a data-toggle="modal" class="request-trip" href="#myModal-host" onclick="contact_host(<?php echo $product->id; ?>);"><?php if($this->lang->line('contact_host') != '') { echo stripslashes($this->lang->line('contact_host')); } else echo "Contact host";?></a> -->
<a class="view-prof" href="#">view full profile</a>
...
<div id="myModal-host" class="modal fade in myModal-host-new" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		   <div class="modal-content" id="pre_approve_reject">
<div class="modal-header" id="address-modal">
	
	<a class="btn btn-default close-btn" data-dismiss="modal" id="pre-approve-close"><span class="">x</span></a>
...
<div id="pre_approve_accept" style="display:block">
 ...
 <div class="top-title">
			 ...
			 <input type="text" placeholder="dd-mm-yyyy" id="datefromContact1" class="checkout ui-datepicker-target" autocomplete="off"/> 
			<!-- <input type="text" placeholder="dd-mm-yyyy" autocomplete="off" name="datefrom" readonly="readonly" id="datefromContact" class="checkin ui-datepicker-target" /> -->
                    ...
                    <li>
						...
						<input type="text" placeholder="dd-mm-yyyy" name="expiredate" autocomplete="off"  id="expiredateContact1" onchange="return DateBetweenContactNew();" class="checkout ui-datepicker-target">
				...
				<div class="service-copmity" id="price_div1" style="display:none">
                                                       
                    <ul>

                    <li><span id="bookingdate1">
...
<label class="price"><?php echo $this->session->userdata('currency_s'); ?></label><label id="bookingsubtot1">
					...
					<p style="font-size:14px" id="stax1">
					...
					echo round(($servicefee * $service_tax->row()->commission_percentage)/100);?></p></center>
					 ...
					 <p id="servicetax1"  style="display:none;"><?php echo $service_tax->row()->commission_percentage; ?></p>

					 <p id="taxtype1" name="taxtype" style="display:none;"><?php echo $service_tax->row()->promotion_type; ?></p>

                    <li ><span><?php if($this->lang->line('total') != '') { echo stripslashes($this->lang->line('total')); } else echo "Total"; ?></span><label class="price"> <?php echo $this->session->userdata('currency_s'); ?></label><label id="bookingtot1" class="table-cell-price">0.00</label></li>

					

					 <li style="display:none;"><span><?php if($this->lang->line('total') != '') { echo stripslashes($this->lang->line('total')); } else echo "Total"; ?></span><label id="bookingtot1" class="table-cell-price"></label></li>
					...
					<li><textarea  name="review1" id="offer_message_contact" class="scroll_newdes" style="height:90px;" onkeypress="return IsEmpty('title');"></textarea>
					
					
					
</li>

<a class="booking-btn" onclick="" href="javascript:contact_host_send();"><?php if($this->lang->line('book_now') != '') { echo stripslashes($this->lang->line('book_now')); } else echo "Book Now"; ?></a>

<!--<a class="request-trip" href="javascript:contact_host_send();"><?php if($this->lang->line('contact') != '') { echo stripslashes($this->lang->line('contact')); } else{ echo "contact"; }?></a> -->
                </ul>
...
function contact_host_send()
{
//alert("inside function");

	    var val = $("#offer_message_contact").val();
    ...
    $.ajax({
		...
		data:{'checkIn':$("#datefromContact1").val(),'checkOut':$("#expiredateContact1").val(),'noOfNyts':$("#bookingdate1").val(),'productPrice':$("#bookingsubtot1").val(),'serviceFee':$("#service_tax").val(),'totalPrice':$("#bookingtot1").val(),'noOfGuests':$("#number_of_guests").val(),'msg':$("#review1").val()},


        success:function(response){
			$("#pre-approve-close").click();
        }
    ...
    $.ajax({
        ...
        success:function(response){
            ...
            if(response=='Date Already Booked'){
				...
				if($("#datefrom1").val()!='' && $("#expiredate1").val()!='') {
